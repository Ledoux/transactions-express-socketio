...
function useTransactionsExpressSocketio(server) {
  ...
  // redux store
  var normalizer = (0, _transactionsReduxNormalizer.createReducer)(description, { definition: definition });
  ...
  nameSpace.on('connection', function (socket) {
    ...
    socket.on('connect_user', function (user) {
      store.dispatch((0, _transactionsReduxNormalizer.mergeNormalizerEntity)('users', user.id, user));
    });
